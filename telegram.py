...
class Telegram(RPCHandler):
    ...
    def _format_entry_msg(self, msg: RPCEntryMsg) -> str:
        ...
        terminology = {
            "1_enter": "עסקה חדשה",
            "1_entered": "עסקה נסגרה",
            "x_enter": "מגדיל עסקה",
            "x_entered": "עסקה הוגדלה",
        }
        ...
        message += f"*סיבת כניסה:* `{msg['enter_tag']}`\n" if msg.get("enter_tag") else ""
        message += f"*כמות מטבעות:* `{round_value(msg['amount'], 8)}`\n"
        message += f"*סוג עסקה:* `{msg['direction']}"
        if msg.get("leverage") and msg.get("leverage", 1.0) != 1.0:
            message += f" ({msg['מינוף']:.3g}x)"
        message += "`\n"
        message += f"*מחיר כניסה:* `{fmt_coin2(msg['open_rate'], msg['quote_currency'])}`\n"
        if msg["type"] == RPCMessageType.ENTRY and msg["current_rate"]:
            message += (
                f"*מחיר כרגע:* `{fmt_coin2(msg['current_rate'], msg['quote_currency'])}`\n"
            )
        ...
        message += f"*{'חדש ' if msg['sub_trade'] else ''}שווי כולל :* `{total}{profit_fiat_extra}`"
    ...
    def _format_exit_msg(self, msg: RPCExitMsg) -> str:
        ...
        cp_extra = ""
        exit_wording = "יצא" if is_fill else "יוצא"
        ...
        message = (
            ...
            f"*{f'{profit_prefix}רווח' if is_fill else f'רווח {profit_prefix}לא ממומש'}:* "
            f"`{msg['profit_ratio']:.2%}{profit_extra}`\n"
            f"{cp_extra}"
            f"{enter_tag}"
            f"*סיבת יציאה:* `{msg['exit_reason']}`\n"
            f"*סוג עסקה:* `{msg['direction']}"
            f"{leverage_text}`\n"
            f"*כמות:* `{round_value(msg['amount'], 8)}`\n"
            f"*מחיר כניסה:* `{fmt_coin2(msg['open_rate'], msg['quote_currency'])}`\n"
        )
        if msg["type"] == RPCMessageType.EXIT and msg["current_rate"]:
            ...
            if msg["order_rate"]:
                message += f"*מחיר יציאה:* `{fmt_coin2(msg['order_rate'], msg['quote_currency'])}`"
        elif msg["type"] == RPCMessageType.EXIT_FILL:
            message += f"*מחיר יציאה:* `{fmt_coin2(msg['close_rate'], msg['quote_currency'])}`"

        if is_sub_trade:
            stake_amount_fiat = self.__format_profit_fiat(msg, "stake_amount")

            rem = fmt_coin(msg["stake_amount"], msg["quote_currency"])
            message += f"\n*כמה נשאר:* `{rem}{stake_amount_fiat}`"
        else:
            message += f"\n*משך:* `{duration} ({duration_min:.1f} דקות)`"
        return message
    ...
    async def _stats(self, update: Update, context: CallbackContext) -> None:
        ...
        reason_map = {
            "roi": "מטרה הושגה",
            "stop_loss": "סטופ לוס",
            "trailing_stop_loss": "טריין סטופ",
            "stoploss_on_exchange": "סטופ לוס",
            "exit_signal": "אות יציאה",
            "force_exit": "יציאה מיידית",
